...
def compute_vote_loss(data_dict, device):
...
def compute_objectness_loss(data_dict, device):
    ...
    objectness_label = torch.zeros((B,K), dtype=torch.long).to(device=device)
    objectness_mask = torch.zeros((B,K)).to(device=device)
    ...
    criterion = nn.CrossEntropyLoss(torch.Tensor(OBJECTNESS_CLS_WEIGHTS).to(device=device), reduction='none')
...
def compute_box_and_sem_cls_loss(data_dict, config, device):
    ...
    heading_label_one_hot = torch.FloatTensor(batch_size, heading_class_label.shape[1], num_heading_bin).zero_().to(device=device)
    ...
    size_label_one_hot = torch.FloatTensor(batch_size, size_class_label.shape[1], num_size_cluster).zero_().to(device=device)
    ...
    mean_size_arr_expanded = torch.from_numpy(mean_size_arr.astype(np.float32)).to(device=device).unsqueeze(0).unsqueeze(0) # (1,1,num_size_cluster,3) 
...
def compute_reference_loss(data_dict, config, device):
    ...
    pred_center = data_dict['center'].detach().cpu().numpy()    # B,num_proposal,3
    ...
    ######################################################################################################
    pred_center = np.tile(pred_center,(data_dict['cluster_ref'].shape[0],1,1))
    pred_heading_class = np.tile(pred_heading_class,(data_dict['cluster_ref'].shape[0],1))
    pred_heading_residual = np.tile(pred_heading_residual,(data_dict['cluster_ref'].shape[0],1))
    pred_size_class = np.tile(pred_size_class,(data_dict['cluster_ref'].shape[0],1))
    pred_size_residual = np.tile(pred_size_residual,(data_dict['cluster_ref'].shape[0],1,1))
    ...
    cluster_labels = torch.FloatTensor(labels).to(device=device)
...
def get_loss(data_dict, config, device, detection=True, reference=True, use_lang_classifier=False, amplify_factor=10):
    """ Loss functions
    ...
    # Vote loss
    vote_loss = compute_vote_loss(data_dict, device)

    # Obj loss
    objectness_loss, objectness_label, objectness_mask, object_assignment = compute_objectness_loss(data_dict, device)
    num_proposal = objectness_label.shape[1]
    total_num_proposal = objectness_label.shape[0]*objectness_label.shape[1]
    data_dict['objectness_label'] = objectness_label
    data_dict['objectness_mask'] = objectness_mask
    data_dict['object_assignment'] = object_assignment
    data_dict['pos_ratio'] = torch.sum(objectness_label.float().to(device=device))/float(total_num_proposal)
    data_dict['neg_ratio'] = torch.sum(objectness_mask.float())/float(total_num_proposal) - data_dict['pos_ratio']

    # Box loss and sem cls loss
    center_loss, heading_cls_loss, heading_reg_loss, size_cls_loss, size_reg_loss, sem_cls_loss = compute_box_and_sem_cls_loss(data_dict, config, device)
    ######################################################################################################
    # box_loss = center_loss + 0.1*heading_cls_loss + heading_reg_loss + 0.1*size_cls_loss + size_reg_loss
    box_loss = center_loss + 0.1*heading_cls_loss + 0.1*heading_reg_loss + size_cls_loss + size_reg_loss
    ######################################################################################################
    ...
    if detection:
        ...
        data_dict['vote_loss'] = torch.zeros(1)[0].to(device=device)
        data_dict['objectness_loss'] = torch.zeros(1)[0].to(device=device)
        data_dict['center_loss'] = torch.zeros(1)[0].to(device=device)
        data_dict['heading_cls_loss'] = torch.zeros(1)[0].to(device=device)
        data_dict['heading_reg_loss'] = torch.zeros(1)[0].to(device=device)
        data_dict['size_cls_loss'] = torch.zeros(1)[0].to(device=device)
        data_dict['size_reg_loss'] = torch.zeros(1)[0].to(device=device)
        data_dict['sem_cls_loss'] = torch.zeros(1)[0].to(device=device)
        data_dict['box_loss'] = torch.zeros(1)[0].to(device=device)

    if reference:
        ref_loss, _, cluster_labels = compute_reference_loss(data_dict, config, device)
        data_dict["cluster_labels"] = cluster_labels
        data_dict["ref_loss"] = ref_loss
    else:
        data_dict["cluster_labels"] = objectness_label.new_zeros(objectness_label.shape).to(device=device)
        data_dict["cluster_ref"] = objectness_label.new_zeros(objectness_label.shape).float().to(device=device)
        data_dict["ref_loss"] = torch.zeros(1)[0].to(device=device)

    if reference and use_lang_classifier:
        data_dict["lang_loss"] = compute_lang_classification_loss(data_dict)
    else:
        data_dict["lang_loss"] = torch.zeros(1)[0].to(device=device)

    ######################################################################################################
    # Final loss function
    #loss = data_dict['vote_loss'] + 0.5*data_dict['objectness_loss'] + data_dict['box_loss'] + 0.1*data_dict['sem_cls_loss'] + 0.1*data_dict["ref_loss"] + 0.1*data_dict["lang_loss"]
    
    # amplify
    # loss *= amplify_factor

    data_dict['detection_loss'] = data_dict['vote_loss'] + data_dict['objectness_loss'] + data_dict['box_loss'] + data_dict['sem_cls_loss']

    loss = data_dict['detection_loss'] + data_dict['lang_loss'] + data_dict['ref_loss']
