...
#-------------------------------------------------------------------------------------------------------------------------------
    ...
    def __getitem__(self, idx):
        ...
        # get pc (## this part is all about the related scene and point clouds)
        ...
        # ------------------------------- LABELS ------------------------------    
        target_bboxes = np.zeros((MAX_NUM_OBJ, 6)) ## center(3) & l,w,h of bboxes
        target_bboxes_mask = np.zeros((MAX_NUM_OBJ)) ## it shows that bbox exist or not(1 for every bbox {0:num_bbox} and 0 for others {num_bbox:MAX_NUM_OBJ})
        angle_classes = np.zeros((MAX_NUM_OBJ,)) ## 0 for every bbox because bboxes are "axis_aligned"! ##
        angle_residuals = np.zeros((MAX_NUM_OBJ,)) ## 0 for every bbox because bboxes are "axis_aligned"! ##
        size_classes = np.zeros((MAX_NUM_OBJ,)) ## semantic class of every bbox (Consider use size2class!)
        size_residuals = np.zeros((MAX_NUM_OBJ, 3)) ## residual size of every bbox in comparison with mean size of that class in all scenes of dataset
        ...
        if self.split != "test": 
            ...
            for i, gt_id in enumerate(instance_bboxes[:num_bbox,-1]): ## 8th element of instance_bbox is instance id
                if gt_id == object_id:
                    ref_box_label[i] = 1
        ...
        data_dict["lang_feat"] = lang_feat.astype(np.float32) # language feature vectors ## (CONF.TRAIN.MAX_DES_LEN, 300)
        ...
        data_dict["ref_heading_class_label"] = np.array(int(ref_heading_class_label)).astype(np.int64) ## ref_heading_class_label = 0
        data_dict["ref_heading_residual_label"] = np.array(int(ref_heading_residual_label)).astype(np.int64) ## ref_heading_residual_label = 0
        data_dict["ref_size_class_label"] = np.array(int(ref_size_class_label)).astype(np.int64) ## semantic 18 class id
...
#-------------------------------------------------------------------------------------------------------------------------------
    ...
    ## this function checks all the objects in one scene
    ## every object has a unique "object_id" and label according to "object_name"
    ## if there was more than one type of object in a scene (e.g. two chairs in one scene) then unique_multiple = 1 (multiple) else 0 (unique)
...
#-------------------------------------------------------------------------------------------------------------------------------
    ...
    def _load_data(self):
        ...
        self.raw2label = self._get_raw2label() ## 40 class mapping (object name to a label) (for example monitor --> otherprop)
