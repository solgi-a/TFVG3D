...
def get_eval(data_dict, config, reference, use_lang_classifier=False, use_oracle=False, use_cat_rand=False, use_best=False, post_processing=None):
    ...
    ###########################################################################
    objectness_preds_batch = objectness_preds_batch.repeat(batch_size,1)
    objectness_labels_batch = objectness_labels_batch.repeat(batch_size,1)
    ###########################################################################
    ...
    ###########################################################################
    pred_center = pred_center.repeat(batch_size,1,1)
    pred_heading_class = pred_heading_class.repeat(batch_size,1)
    pred_heading_residual = pred_heading_residual.repeat(batch_size,1)
    pred_size_class = pred_size_class.repeat(batch_size,1)
    pred_size_residual = pred_size_residual.repeat(batch_size,1,1)

    gt_center = gt_center.repeat(batch_size,1,1)
    gt_heading_class = gt_heading_class.repeat(batch_size,1)
    gt_heading_residual = gt_heading_residual.repeat(batch_size,1)
    gt_size_class = gt_size_class.repeat(batch_size,1)
    gt_size_residual = gt_size_residual.repeat(batch_size,1,1)
    ###########################################################################
