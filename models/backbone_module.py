...
class Pointnet2Backbone(nn.Module):
    ...
    def _break_up_pc(self, pc):
        ...
        features = pc[..., 3:].transpose(1, 2).contiguous() if pc.size(-1) > 3 else None # (B,C,N)
...
if __name__=='__main__':
    ...
    device = 'cpu'
    backbone_net = Pointnet2Backbone(input_feature_dim=3).to(device)
    print(backbone_net)
    backbone_net.eval()
    
    random_numbers = torch.rand(2,4096,6).to(device)
    
    data_dict = {"point_clouds" : random_numbers}
